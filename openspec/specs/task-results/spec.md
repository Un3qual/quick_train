...
Collection context SHALL be exposed through Tasks-owned read-only Ash resources over the canonical tables, with explicit field and relationship allowlists and no persisted copies. Forms and Datasets resources SHALL retain ordinary domain authorization and SHALL not depend on task-specific policy checks or read actions. Every collection context traversal SHALL recheck live worker or result authority, including exact issued input and bound field membership; caller-supplied traversal context alone SHALL confer no authority.
