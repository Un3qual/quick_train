...
The system SHALL accept a caller-supplied idempotency key and associate it with a SHA-256 fingerprint of the schema version and immutable open parameters, persisted in PostgreSQL as exactly 32 binary bytes. Organization, dataset, and idempotency key SHALL form a unique identity. Matching sequential or concurrent requests SHALL return one import, while reusing that identity for changed parameters SHALL fail with `idempotency_conflict`.
...
### Requirement: Expired open imports reject work and remain stored
The system SHALL reject append and first finalization after an open import expires. Expired imports, rows, and candidates SHALL remain stored and retain their batch identity until cleanup is restored in `restore-operator-bootstrap-and-maintenance`.
...
- **THEN** the system returns a bounded keyset Relay row-outcome connection whose entries expose provenance, sanitized failures, and resulting typed item revisions without requiring append-request replay
