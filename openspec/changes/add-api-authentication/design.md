...
Discovery metadata and verification keys may be reused only after the complete provider configuration has passed these endpoint and S256 checks. The cached client context expires at the earliest provider-advertised discovery or key-set cache deadline; refresh failure fails the request closed instead of serving stale provider material.
