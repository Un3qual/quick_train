...
Discovery metadata and verification keys may be reused only after the complete provider configuration has passed these endpoint and S256 checks. The cache contains only that validated public provider material, keyed by issuer, and expires it at the earliest provider-advertised discovery or key-set cache deadline. Each request constructs its credential-bearing client context outside the cache. Refresh failure fails the request closed instead of serving stale provider material.
...
The clean-database authentication migration is intentionally forward-only because its one-way credential hashes and issuer/subject identity cannot be reconstructed as the legacy schema. Before product data exists, rollback means revoking issued credentials and resetting the local database. Once product domains rely on bearer sessions, rollback requires a deliberate replacement-authentication migration and is not an automatic schema downgrade.
