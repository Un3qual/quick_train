...
- [ ] 3.1 Enforce trusted-proxy-aware HTTPS for production begin, exchange, and callback traffic, reject insecure callback configuration outside exact nonproduction loopback use, implement indexed bearer resolution, and install the active global user as both the Absinthe and Ash actor without selecting organization scope.
...
- [ ] 3.3 Add the responsibility-named operator-only first-manager Mix task using transactional Ash actions and the exact user, organization, membership, `manager` role, assignment, and two foundation-capability identities, including deterministic conflict and concurrent-idempotency handling.
...
- [ ] 4.4 Add tests for the exact first-manager graph, conflict atomicity, and concurrent idempotency plus automatic login-state and inactive-session cleanup that preserves live credentials and avoids overlapping jobs.
