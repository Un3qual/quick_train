...
- [ ] 3.1 Enforce trusted-proxy-aware authentication transport at the Phoenix boundary: permit only proof-free begin redirects before login-material creation; hard-reject cleartext exchange, callback, and bearer traffic without redirect and before provider, token, or GraphQL work; reject insecure callback and provider endpoints; mark credential-bearing responses `Cache-Control: no-store`; resolve bearer sessions by indexed hash; and install the active global user as both the Absinthe and Ash actor without selecting organization scope.
