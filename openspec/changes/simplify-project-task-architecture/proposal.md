...
- Expose native Ash create/update project mutations, native child configuration upsert/destroy actions, native work-bundle reads, and native attempt updates including draft saves and submission. Batch export source reads and compute receipt totals with native Ash aggregates. Centralize scoped locked reads and pending-asset staging setup through domain interfaces. Remove duplicate task identity data, bulk-expire overdue leases, and derive the cohort from authored task inputs. Unsealed export snapshots may retry after transient failures. Pin source identities at creation; drafts configure their tasks, bindings, slot policies, and project-level submission/skip settings.
