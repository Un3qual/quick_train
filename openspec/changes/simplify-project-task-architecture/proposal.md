...
- Export complete project results in accepted or audit mode. Pin each selected submitted outcome and its effective review decision once, deriving immutable related records from those owners. Audit outcomes with no decision retain null decision IDs and empty review history on retry. Remove arbitrary record-kind and UUID-range partitioning.
- Expose native Ash create/update project mutations, native child configuration upsert/destroy actions, native work-bundle reads, native attempt updates including draft saves and submission, and native export-request creates. Batch export source reads and compute receipt totals with native Ash aggregates. Centralize scoped locked reads and pending-asset staging setup through domain interfaces. Remove duplicate task identity data, bulk-expire overdue leases, and derive the cohort from authored task inputs. Unsealed export snapshots may retry after transient failures. Pin source identities at creation; drafts configure their tasks, bindings, slot policies, and project-level submission/skip settings.
...
- Sanitize source-download failures, apply the existing asset byte cap during export generation, and remove unused indexes from the unreleased schema.
- Recover failed or expired export uploads without changing the sealed snapshot or its content, retaining ownership of superseded assets.
