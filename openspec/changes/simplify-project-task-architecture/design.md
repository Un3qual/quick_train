...
Generate migrations with Ash codegen, then add the data transfer needed before removing redundant fields/tables. Retain historical migrations. Test with existing evidence in a disposable database and run the complete repository gate. Production rollback retains evidence and uses a forward fix; destructive down migrations remain a disposable-database operation.
