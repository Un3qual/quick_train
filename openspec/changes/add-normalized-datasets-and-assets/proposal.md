...
- Add idempotent import batches and rows with bounded normalized relational staging, stable keyless-item targeting, source provenance, partial completion, open-batch expiry, row-derived progress, atomically scheduled retry-safe row processing, and narrow reconciliation of terminal Oban jobs, but no custom processing leases or persisted aggregate counters.
- Add deliberate GraphQL actions for dataset and schema lifecycle, programmatic import with paginated row-outcome inspection, asset registration and finalization, authorized asset access, and organization-scoped typed reads with navigable product relationships.
