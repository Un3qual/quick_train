...
- [ ] 1.3 Add exact capability keys `assets.read`, `assets.manage`, `datasets.read`, `datasets.manage`, and `dataset_imports.manage`; add a product-owned idempotent action that grants them to a selected existing organization manager without a shared manifest or wildcard; and add shared fail-closed policy checks for active organization, active membership, and explicit capability.
