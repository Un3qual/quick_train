...
#### Scenario: NUL text is rejected before persistence
- **WHEN** an import row or direct revision supplies text containing a NUL character
- **THEN** normalization rejects it before reserving row identity or constructing a record, preserving the ability to retry corrected input
