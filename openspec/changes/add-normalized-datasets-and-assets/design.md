...
`Asset` contains organization ownership, lifecycle state, canonical lowercase hexadecimal SHA-256, byte size, media type, optional image dimensions, internal staging and sealed keys, staging expiry, cleanup completion, and timestamps. Ready assets are immutable. A partial unique identity on organization and content hash applies only to ready assets so a failed upload does not permanently reserve the identity.

Registration rejects a declared byte size above the configured limit before returning access to a unique writable staging object. Initial repository defaults are 100 MiB per asset, 16,384 pixels for either image dimension, and 100,000,000 total image pixels; deployment configuration may lower or deliberately raise those positive limits. The storage descriptor applies a provider-side upload cap when supported. Finalization checks sealed-object size before reading content, parses image headers with bounded reads, rejects excessive dimensions or pixel counts before full decode, and never relies on decompression to discover limits.

One idempotent adapter operation pins a staged version or fences further writes, seals exactly those bytes at the organization-and-SHA-256 canonical immutable key, and returns facts computed from that same sealed object. A conditional create or equivalent compare-and-converge operation ensures concurrent identical content resolves the same sealed location; an existing canonical object must be reverified against the exact bytes before reuse. If the provider cannot pin staging, it must irrevocably fence writes before sealing and verify the canonical sealed object afterward. A per-registration sealed copy or separate unguarded verify-then-copy sequence is invalid.

Finalization verifies hash, size, safely detected media type, and image dimensions from the sealed object. Active formats such as HTML, XHTML, and SVG are rejected even under a benign declaration. Concurrent identical finalizations converge on one canonical ready asset and one canonical sealed object only when size and media type also match; the losing matching registration becomes `duplicate_content`, points to the canonical asset for resolution, owns no sealed copy, and leaves only its redundant staging object cleanup eligible. Cross-organization content never shares authorization or sealed locations.
...
A periodic responsibility-named cleanup worker scans expired rows whose `staging_cleaned_at` is null. Its cleanup action acquires the same asset row lock as finalization, rechecks lifecycle and expiry, and holds that lock across idempotent staging deletion or absence confirmation and the cleanup marker commit. Finalization holds the lock across verify-and-seal and its terminal transition, so cleanup cannot remove staging from an in-flight finalization. Retries before the marker remain safe; completed rows leave the indexed scan; sealed read objects are never deleted.
...
The GraphQL append action accepts one flat row per call. Safe repository defaults cap each import at 100,000 accepted rows, each row at 256 field entries and 1 MiB of accepted scalar bytes, and each individual text value at 256 KiB; deployment configuration may lower or deliberately raise those positive limits. The fixed input type rejects arbitrary nesting and unsupported structures. Request/body, row-count, field-count, and byte limits are checked before canonicalization, sorting, hashing, candidate construction, or row-identity reservation.
...
Append and finalization lock the same import. An import persists only `open` or `sealed` phase plus immutable `open_expires_at`. First finalization seals the row set and inserts one unique batch job. A periodic cleanup worker deletes only expired still-open imports after locking and rechecking them; sealed provenance is never deleted.

The batch worker idempotently enqueues one unique bounded-retry row job for each pending row. A row job locks its row, skips terminal outcomes, and atomically commits item revision and terminal outcome. Failure before commit leaves the row pending for ordinary Oban retry. Failure after commit but before job acknowledgement is safe because the retry observes the terminal row.

A single responsibility-named import terminalizer runs on a fixed unique periodic schedule and uses the durable Oban job state as the retry authority. It scans terminal row-job records in bounded pages, locks a still-pending row, and records sanitized `processing_retries_exhausted` failure only after its unique row job is discarded or cancelled and no available, scheduled, retryable, or executing attempt remains. Oban pruning retains discarded and cancelled row-job evidence longer than the maximum terminalizer interval, and the terminalizer completes the row before that evidence becomes prune eligible. Row processing and terminalization therefore race through the same row lock. This narrow batch-level reconciliation is not a processing scheduler: there is no persisted `processing` state, custom lease, row attempt counter, attempt fence, or per-attempt recovery job.
...
`add-api-authentication` is implemented first. This change adds exact capability keys `assets.read`, `assets.manage`, `datasets.read`, `datasets.manage`, and `dataset_imports.manage`, extends the deterministic manager bootstrap manifest with those grants, and requires each product action to derive its organization from the target resource or explicit relationship before checking the matching capability. Internal jobs advance only the immutable organization, dataset, schema, asset, or import scope accepted by an authorized caller; they do not grant read access or broaden scope.
...
This change reuses the stable pinned Oban dependency established by the authentication prerequisite. Workers are responsibility-named for asset verification, asset staging cleanup, import fan-out, import-row processing, and expired-open-import cleanup. Jobs carry only resource identities, not dataset content. Job uniqueness prevents duplicate fan-out and overlapping periodic cleanup, while application actions remain independently idempotent.
...
- **[A repeatedly failing row exhausts Oban retry policy]** -> Keep row work idempotent and use one discarded-job terminalizer to convert only terminally abandoned pending rows into sanitized failures without leases or per-attempt state.
...
6. Add import open, append, finalize, cleanup, batch fan-out, idempotent row processing, discarded-job terminalization, and derived progress queries.
