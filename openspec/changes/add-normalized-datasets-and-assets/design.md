...
The first release supports exactly one configured account-linking policy, `issuer_subject_only`. Missing or unknown policy configuration fails closed before login can proceed. The canonical identity is the verified OIDC issuer URI plus subject from a provider response whose signature, issuer, audience, nonce, and expiry checks passed. An existing identity resolves only its immutable user relationship; refresh may update non-identity claims but can never upsert `user_id`. A new identity requires a non-empty provider-verified email to create a new global user, but email is never used to link or reassign an existing user. If that email already belongs to another user, or any issuer/subject/user identity conflicts, exchange returns `account_linking_conflict` without issuing a session; later policies may add explicit linking without weakening this default.
...
2. Complete the OIDC-to-bearer-session handshake with server-generated state, nonce, and S256 PKCE, enforce the `issuer_subject_only` linking policy, add periodically scheduled bounded login-transaction retention, make session token hashes required and uniquely indexed, add the authenticated GraphQL actor pipeline, active-organization checks, capability definitions, and first-manager operator bootstrap, remove unauthenticated GraphQL health, and add fail-closed tests before registering product domains in the schema.
