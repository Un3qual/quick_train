...
Finalization verifies hash, size, safely detected media type, and image dimensions from pinned staging before canonical publication and from the sealed object afterward. Plain text excludes tag-like markup, rather than using a list of selected HTML elements. Active formats such as HTML, XHTML, and SVG are rejected even under a benign declaration. PNG inspection checks the complete IHDR, chunk framing and CRCs, image-data presence, and terminal IEND without decompressing pixels; it is not a full image decoder. GIF is unsupported until complete format validation is available; dimensions alone are insufficient. PDF headers anywhere within the first 1,024 bytes are rejected, including whitespace and BOM prefixes. PDFs are unsupported: recognizing a PDF header does not establish that it lacks embedded scripts, and this change does not add a PDF parser or sanitizer. Concurrent identical finalizations converge on one canonical ready asset and one canonical sealed object only when size and media type also match; the losing matching registration becomes `duplicate_content`, points to the canonical asset for resolution, owns no sealed copy, and retains its redundant staging object. Cross-organization content never shares authorization or sealed locations. Looking up an existing ready canonical asset needs no row lock because its content and lifecycle are immutable. The pending registration lock, claim checks, and ready-content unique identity still arbitrate concurrent finalization.
