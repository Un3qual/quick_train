...
Registration returns access to a unique writable staging object. One idempotent adapter operation pins a staged version or fences further writes, seals exactly those bytes at an immutable key or provider version, and returns facts computed from that same sealed object. If the provider cannot pin staging, it must irrevocably fence writes before sealing and verify the sealed object afterward. A separate unguarded verify-then-copy sequence is invalid.

Finalization verifies hash, size, safely detected media type, and image dimensions from the sealed object. Active formats such as HTML, XHTML, and SVG are rejected even under a benign declaration. Concurrent identical finalizations converge on one canonical ready asset only when size and media type also match; the losing matching registration becomes `duplicate_content` and its redundant staging object is cleanup eligible. Cross-organization content never shares authorization.
...
A periodic responsibility-named cleanup worker scans expired rows whose `staging_cleaned_at` is null. After provider deletion or confirmed absence, it locks the asset and records completion. Retries before the marker remain safe; completed rows leave the indexed scan; sealed read objects are never deleted.
...
The GraphQL input type accepts only bounded flat field entries and supported scalar representations. Malformed scalar shapes, arbitrary nested input, and unsupported structures fail before a row identity is reserved. After structural acceptance, append computes a versioned fingerprint from the schema identity, row key, external-key marker, source position, and canonical accepted field entries. It preserves domain-invalid keys, selectors, duplicates, and scalar values without needing a total encoding for every possible malformed request tree. Equivalent field order, decimal scale, and UTC offsets converge.
...
`add-api-authentication` is implemented first. This change adds asset and dataset capability keys and requires each product action to derive its organization from the target resource or explicit relationship, then require an active organization, active membership, and matching capability. Internal jobs advance only the immutable organization, dataset, schema, asset, or import scope accepted by an authorized caller; they do not grant read access or broaden scope.
...
- **[A repeatedly failing row relies on Oban retry policy]** -> Keep row work idempotent, configure bounded retry and discard behavior, and record a sanitized terminal failure for nontransient domain errors.
...
6. Add import open, append, finalize, cleanup, batch fan-out, idempotent row processing, and derived progress queries.
