...
- Use responsibility-specific durable jobs for login-state cleanup, asset verification, staging cleanup, and import processing.
...
Storage credentials and persistent object locations are never GraphQL fields. GraphQL returns only short-lived adapter-produced access descriptors after an Ash authorization action succeeds. Every credential-bearing descriptor uses authenticated encrypted transport, rejects insecure schemes and redirects, and carries no-store/no-referrer handling so credentials are not propagated through caches or referrer metadata.
...
One responsibility-owned canonical encoder supplies both revision content fingerprints and import request fingerprints. Its versioned, domain-separated byte stream uses unambiguous length-prefixed segments; includes the schema-version and root-record-type UUID bytes; and orders occurrences by field-definition UUID bytes and then ordinal, independent of caller field order. Each occurrence includes its field UUID, value-family tag, ordinal, and canonical typed bytes: exact validated UTF-8 for text; minimal signed base-10 for integers; non-exponent decimal form with insignificant trailing zeroes and negative zero removed; one byte for booleans; signed UTC Unix microseconds for date-times; and immutable asset UUID bytes for assets. Absent optional values emit no occurrence. SHA-256 of that stream is represented as 64 lowercase hexadecimal characters. The import-row fingerprint uses a separate domain tag but embeds the same canonical record stream alongside explicit null/present markers and canonical encodings for the row key, external key, source position, and every other immutable row parameter. Equivalent field order, decimal scale, time-zone offset, or input occurrence order therefore converges, while a schema, field, ordinal, family, or actual value change diverges.
...
Every appended row supplies and persists a caller-stable row key unique within the import, a customer external key, a unique source position, and a fingerprint produced by the shared canonical encoder over its normalized input plus all immutable row parameters, including that external key. Append uses the same atomic create-or-return rule. A matching retry returns the already accepted row without reconstructing or reprocessing it; reuse of the row key or source position for changed content or a changed external key fails with `idempotency_conflict`. A partial database identity on import and non-null external key reserves each target item at most once in a batch. The first committed append wins that identity and every competing duplicate is rejected with `duplicate_external_key`, so worker scheduling cannot choose which of two rows becomes the final value.
...
Appending and finalizing serialize on the same locked `DatasetImport` row. Append rechecks that the locked import is `open` before it persists a row; finalization locks that row and atomically seals it into the derived post-finalization lifecycle before it enqueues processing. The worker therefore sees a sealed row set, and an append that loses the race is rejected with `import_not_open` rather than appearing after completion.

An import is `open` until finalization. After sealing, its lifecycle is derived with fixed precedence from its rows: any unexpired processing lease yields `processing`; otherwise any pending row or expired processing lease yields `pending`; otherwise zero rows or terminal rows with no failures yield `completed`; terminal rows that all failed yield `failed`; and any other terminal mix of failed with succeeded or unchanged rows yields `partially_failed`. Counts are direct, disjoint persisted-row counts: `accepted` is every persisted `DatasetImportRow`; `pending`, `processing`, `succeeded`, and `unchanged` count rows in those exact outcomes; and `failed` counts every persisted terminal failure, including append-validation and processing failures. A processing row remains in the processing count even when its expired lease makes the batch retryable and therefore `pending`. A competing `duplicate_external_key` append is rejected before row persistence, so it is neither accepted nor counted as a failed row.

Workers claim pending rows or processing rows whose lease has expired, recording `processing_started_at`, `lease_expires_at`, and an incremented attempt count under a row lock. The returned attempt number is a fencing token: a terminal update succeeds only when the row is still processing under that exact attempt. A retry skips terminal rows and active leases but safely reclaims stale processing rows; a stalled older attempt cannot overwrite a newer attempt's outcome after resuming. Before a worker exits while any active lease remains, it snoozes the current job or transactionally inserts one unique follow-up scheduled for the earliest lease expiry, guaranteeing automatic reclamation without an external invocation. Rows targeting the same stable item serialize on its row lock after the atomic item get-or-create step.
...
The existing OIDC helpers and login-transaction resource do not yet complete a client authentication handshake. This change adds unauthenticated begin/exchange actions. Begin generates the state, nonce, and PKCE verifier server-side with a cryptographically secure random generator using at least 256 bits of entropy for each value, stores a unique collision-checked state hash plus the nonce hash and server-side verifier, derives an S256 challenge from a standards-compliant verifier, and rejects caller-supplied state, nonce, verifier, or challenge material. Exchange atomically claims an unexpired login transaction through a one-way `pending` to `exchanging` transition before any provider request may continue.

The first release supports exactly one configured account-linking policy, `issuer_subject_only`. Missing or unknown policy configuration fails closed before login can proceed. The canonical identity is the verified OIDC issuer URI plus subject from a provider response whose signature, issuer, audience, nonce, and expiry checks passed. Before issuer-only lookup is enabled, a staged data migration maps every legacy `provider == "oidc"` identity to the one configured canonical issuer URI and aborts on missing/multiple issuer configuration or any issuer/subject or user/provider conflict; no ambiguous row is guessed or silently relinked. An existing identity resolves only its immutable user relationship when both the identity and linked global user are active; exchange locks and rechecks both lifecycle states in the same transaction that creates the session and consumes the login transaction. Provider claim refresh may update non-identity claims but can never change lifecycle status or upsert `user_id`, so a disabled identity cannot reactivate itself during login. A new identity and user begin active and require a non-empty provider-verified email. The user display name is the first nonblank trimmed `name` or `preferred_username` claim, falling back deterministically to the normalized verified email's nonempty local part; display-name input never participates in account linking. Email is never used to link or reassign an existing user. If the matched identity or user is disabled or has an unknown status, exchange fails closed with a non-disclosing authentication error and no session. If the verified email already belongs to another user, or any issuer/subject/user identity conflicts, exchange returns `account_linking_conflict` without issuing a session; later policies may add explicit linking without weakening this default.

Only the request that wins the conditional login-transaction claim may apply that policy and issue a high-entropy opaque bearer token exactly once; every concurrent or later claimant is rejected, and a failed or interrupted claimant cannot make the state reusable. Session creation and the final `consumed` transition commit together. Only the token's SHA-256 hash, expiry, method, and user relationship are stored in `Session`; issued hashes are non-null and protected by a unique database identity and index for one-row hot-path lookup. Before adding that non-null constraint and identity, a staged migration marks every legacy null-hash session revoked and assigns it a distinct reserved `legacy-revoked:<session-id>` value that bearer hashing cannot produce, preserving audit rows without creating an authenticating credential.
...
Oban is added at the stable GA version selected during implementation and pinned exactly while `.mise.toml`, `mix.exs`, and `mix.lock` are updated together. Its supported migration generator creates the jobs-table migration before workers are enabled. Initial workers are responsibility-named: `QuickTrain.Accounts.CleanupOidcLoginTransactionsWorker`, `QuickTrain.Assets.VerifyWorker`, `QuickTrain.Assets.CleanupStagingWorker`, and `QuickTrain.Datasets.ImportWorker`. Job insertion is idempotent and, when coupled to a state transition, occurs transactionally with the application record. OIDC transaction cleanup and asset staging cleanup are configured in Oban's supported periodic scheduler rather than relying on a caller, finalization attempt, or manual run; each job's uniqueness window prevents overlapping executions while its cleanup action remains independently idempotent.
...
2. Migrate legacy `provider == "oidc"` identities to the one configured canonical issuer URI with conflict detection, revoke and assign reserved non-authenticating values to legacy null-hash sessions, then complete the OIDC-to-bearer-session handshake with server-generated state, nonce, and S256 PKCE, deterministic display-name fallback, the `issuer_subject_only` linking policy plus active external-identity and active-user checks, periodically scheduled bounded login-transaction retention, required uniquely indexed session token hashes, the authenticated GraphQL actor pipeline, active-organization checks, capability definitions, first-manager operator bootstrap, development-only GraphiQL, and fail-closed tests before registering product domains in the schema.
...
4. Generate AshPostgres migrations and snapshots, including dataset/schema, exact-record-type, and designated-root-type composite constraints, ready-asset partial uniqueness, import idempotency and lease constraints, the deferred typed-child constraint trigger, and required indexes.
