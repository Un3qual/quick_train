...
- **[Unauthenticated login starts can accumulate state]** → Persist a retention cutoff and delete expired or consumed login transactions after a fixed replay-rejection interval through an idempotent responsibility-named worker.
...
2. Complete the OIDC-to-bearer-session handshake, add bounded login-transaction retention, make session token hashes required and uniquely indexed, add the authenticated GraphQL actor pipeline and capability definitions, remove unauthenticated GraphQL health, and add fail-closed tests before registering product domains in the schema.
