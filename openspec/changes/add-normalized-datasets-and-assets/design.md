...
`Asset` contains organization ownership, lifecycle state, canonical SHA-256, byte size, media type, optional image dimensions, internal staging and sealed keys, staging expiry, cleanup completion, the minimal bounded operation-claim facts, and timestamps. The shared Ash digest type keeps lowercase hexadecimal values at domain, GraphQL, and storage-adapter boundaries while dumping and loading exactly 32 binary bytes through AshPostgres. Ready assets are immutable. A partial unique identity on organization and content hash applies only to ready assets so a failed upload does not permanently reserve the identity.
