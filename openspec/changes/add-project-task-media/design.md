...
Generate BoundingBox, PolygonRegion/PolygonPoint, MaskRegion, and attempt/question mask-attachment resources inside `lib/quick_train/tasks/`. Keep TaskInput, exact source DatasetValue, and question label references within the same task/form-version boundaries using AshPostgres composite references and ordinary local checks. No JSONB geometry. The existing Response parent lock, expected revision, live-lease check, and submitted immutability govern every child create/update/delete and replacement.

Boxes use finite normalized decimals and strictly positive width/height. Polygons are simple nonzero-area rings with at least three distinct ordered points, implicit closure, no crossings/repeated closing point, and coordinates in `[0,1]`. Multiple regions are supported; a region does not encode holes or a multipolygon. Preserve published annotation counts, plus at most 1,000 regions/question and 1,000 points/polygon. Modify core's `Response writes are bounded and explicitly exposed` requirement here to replace its text-only total with one 10,000-row annotation budget per response: each TextSpan, BoundingBox, PolygonRegion, PolygonPoint, and MaskRegion counts once. This extends the existing Response validation and lock; it adds no counter resource. Keep the 1,000-span/question limit. Zero regions is a valid answer only where minimum zero allows it.
...
Add deliberate Tasks mask registration/finalization/attachment actions tied to an offered raster question and a live eligible owned attempt. Assign the project's organization and retain existing enforced byte caps, hash/size verification, immutable sealing, and canonical deduplication. Store attachment authority from registration onward, including pending assets, and maintain it when finalization selects a canonical asset. A same-organization UUID or known hash alone never authorizes another worker's mask.

Require a 1–128 byte request key unique per attempt/question, bound to immutable hash/size/media arguments. After current owner/eligibility/lease authorization and under the existing Response lock, look up the attempt/question key first. Matching existing keys reuse the registration and return its state-specific outcome without additional allowance even at a limit; changed arguments conflict. For absent keys, validate declared facts and check existing canonical identity first: a different byte size or media type for the same canonical hash returns `asset_identity_conflict` before admission, without reserving the key or allowance or creating a registration, Asset, or staging object. Apply admission limits only to new keys within each attempt lifetime: at most 1,000 registrations for each offered raster question, 10,000 registrations across all its questions combined, and 256 MiB aggregate declared bytes across those registrations, also enforcing the current per-file cap. Count every registration state and replacement without refunds, so repeated finalization or abandonment cannot evade the bound while staging cleanup remains deferred. Commit each new registration/key and allowance atomically with a new pending Asset and reserved staging identity, then perform I/O outside the transaction. Every new key verifies its own uploaded bytes, including when matching canonical content exists. Successful finalization establishes attachment authority for that offered attempt/question and resolves canonical content under existing deduplication rules; subsequent same-key retries reuse that result. This removes the direct ready-asset shortcut and its separate authorization decision. Admitted calls use that Asset rather than rerunning general registration; a canonical asset appearing after the initial check is handled through the pending Asset's existing finalization/deduplication/conflict path. Identical retries of pending registrations resume interrupted storage work using that identity/destination under the existing sealing rules while staging access remains valid, without extra allowance or lease/staging-expiry extension. Terminal failed or expired registrations return their recorded outcome without new upload access or revival; retrying the upload requires a fresh key, current attempt authorization, and remaining allowance, with no refund for the old registration. Ready and deduplicated registrations keep their existing canonical-asset resolution. Admission failures, including `mask_registration_limit`, create no registration/key reservation, consume no allowance, and allocate no asset or writable object. Use the existing attachment/registration records and Response serialization to derive usage; no separate quota service or cleanup subsystem. Workers must fit successful masks and replacement attempts within this finite allowance.

Derive registration outcomes from the existing Assets lifecycle: committed `content_mismatch` or `asset_identity_conflict` means failed, failed `staging_expired` means expired, and ready or `duplicate_content` resolves canonical ready content. Keep the existing asset lock, finalizer-claim, and expiry checks. Transport errors, storage timeouts, missing staging, and interrupted requests leave the registration pending unless a terminal Assets outcome has committed; a lost success response resolves the committed canonical result. This requires no separate failure classifier or registration lifecycle.
