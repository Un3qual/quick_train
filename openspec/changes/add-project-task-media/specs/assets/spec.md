...
A live eligible attempt owner SHALL be able to register/finalize an organization-owned mask asset only through a response workflow identifying an offered raster-mask question. The workflow SHALL preserve the existing enforced upload cap, exact hash/size/media identity, immutable sealing, and canonical deduplication rules and record explicit attempt/question ownership. Pending registration links SHALL preserve ownership and result-access scope while authorizing only scoped upload/finalization work, never attachment, download, or rendering. Worker attachment and content access SHALL require successful finalization of that registration's verified upload plus the existing lease, authorization, and applicable media checks. Registration SHALL require a caller-supplied UUID request key unique within attempt/question: identical retries SHALL reuse the same registration and return its state-specific outcome defined below, while changed hash/size/media arguments SHALL return `idempotency_conflict`. Same-key retries SHALL not create additional writable objects or extend access beyond the original staging expiry or attempt lease.

Registration SHALL serialize under the core Project/Task/Attempt lock order, recheck current owner/eligibility/lease/state after locking, and atomically persist the caller-supplied UUID key, Ash-generated registration identity, and a new pending Asset with an Ash-generated ID and reserved staging identity before storage I/O. Every new key SHALL require verification of its own uploaded bytes even when a matching canonical asset exists; a supplied hash or asset ID SHALL not grant attachment authority. Successful finalization SHALL establish authority only for this registration's offered attempt/question and resolve canonical ready content under the existing deduplication rules. Same-key retries SHALL retain that canonical resolution. Finalization SHALL neither replace response children nor revoke other finalized registrations for that question. Registration records SHALL preserve provenance and attachment eligibility; current answer membership SHALL come only from MaskRegion children selected by the revision-checked whole-question save, with submitted membership immutable. Requests rejected for invalid arguments or insufficient current authority SHALL create no key reservation, Asset, or writable staging object. Once admission commits, a failed access request, including an invalid descriptor, SHALL return no descriptor or credentials and preserve that same pending registration and Asset under the provider-neutral access contract; it SHALL not undo admission or grant attachment authority. Storage I/O SHALL remain outside the database transaction. No task-specific registration-count, aggregate-byte allowance, quota accounting, or automatic staging-deletion prerequisite SHALL be introduced; existing per-file Assets/provider behavior SHALL remain in force.

After current authorization and the core mutation locks, registration SHALL look up the attempt/question UUID request key. An existing key SHALL resolve its matching registration; changed arguments SHALL return `idempotency_conflict`. For an absent key, registration SHALL validate the supplied declared facts under the existing upload rules without looking up canonical content. A valid new request SHALL create its pending registration and Asset regardless of whether the organization already holds that hash or has different canonical size/media facts. Admission outcomes and returned metadata SHALL not disclose canonical existence or facts. Calls SHALL use the linked pending Asset rather than the general registration lookup/reuse shortcut. Canonical deduplication or `asset_identity_conflict` SHALL be resolved only after that registration's own pinned staging bytes pass hash/size verification, whether the canonical asset existed before admission or appeared concurrently. Missing or mismatched staging SHALL retain its existing upload failure outcome without revealing canonical content. Retry convergence SHALL not bypass current authorization or renew expired storage access.

An admitted registration SHALL follow the existing Assets lifecycle. It SHALL become terminally failed when Assets commits `content_mismatch` or `asset_identity_conflict`, and its expired outcome SHALL correspond to Assets committing failed `staging_expired`. Those transitions SHALL retain the existing asset lock, finalizer-claim, and staging-expiry checks. Invalid access descriptors, transport errors, storage timeouts, missing staging, and interrupted requests SHALL leave the registration pending unless a terminal Assets outcome has committed; retries SHALL respect the existing finalizer claim. Ready and `duplicate_content` outcomes SHALL resolve the canonical ready asset, including after a lost success response.
...
#### Scenario: Candidate hashes do not reveal canonical existence
- **WHEN** an eligible external worker supplies otherwise valid new keys and declared facts naming absent content, matching canonical content, or canonical content with different size/media facts
- **THEN** each request admits its own pending registration with the same kind of staging access and no canonical metadata or conflict disclosure before verifying its own upload

#### Scenario: Verified bytes expose a canonical identity conflict
- **WHEN** a registration's own pinned upload passes hash/size verification and canonical content with a different declared media type exists, whether published before or after admission
...
#### Scenario: Unverified staging does not expose a canonical conflict
- **WHEN** a worker finalizes missing staging or bytes that fail the declared hash/size checks, while the organization has canonical content for the declared hash with different facts
- **THEN** the workflow returns the same staging/content failure as for absent canonical content, without disclosing a canonical conflict or granting attachment authority
